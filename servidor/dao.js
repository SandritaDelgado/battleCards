...
function Dao(){
	...
	this.conectar=function(callback){
        var dao=this;
        mongo.connect("mongodb://sandra:sandradelgado1992@ds251727.mlab.com:51727/battlecards",{useNewUrlParser:true}, function(err, db) {
            if (err){
                console.log("No pudo conectar a la base de datos")
            }
            else{
            	console.log("conectado a Mongo: usuarios");
            	const mydb = db.db("battlecards");
                mydb.collection("usuarios",function(err,col){
                    if (err){
